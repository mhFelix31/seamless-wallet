...
def test_all_checks_happy_path(check_param, default_client):
    ...
    assert response_json["services"][check_param] == "ok", f"Checked {check_param}, is not ok"

@pytest.mark.e2e
def test_all_checks_with_failed_parameter(mock_setting, client_factory):
    mock_setting.database_type = "postgres"
    mock_setting.database_url = "postgresql+psycopg://postgres:password@invalid:5432/app"
    ...
    assert response.status_code == 503, f"Status expected is 503, received {response.status_code}"
