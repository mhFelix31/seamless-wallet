...
class TransferHandler:
    ...
    def withdraw(
        ...
        balance = self.get_post_transaction_balance(
            transfer.currency_code, transfer.wallet_id
        )
    ...
    def get_post_transaction_balance(self, currency_code, wallet_id) -> Money:
        currency = get_currency_from_code(
            currency_code=currency_code, currency_repo=self.currency_repo
        )
    ...
    def create_transaction(
        self, source_id: str, receiver_id: str, currency_code: str, value: int
    ) -> Transaction:
        ...
        if not source_wallet.is_external_managed:
            cur_balance = get_wallet_cur_balance(
                snapshot_repo=self.snapshot_repo, wallet_id=source_id
            )
            balance_after_transaction = (
                cur_balance.get(currency, Money(currency=currency, value=0)) - amount
            )
        ...
        transaction = Transaction(
            source=source_wallet, receiver=receiver_wallet, amount=amount
        )
...
def get_currency_from_code(
    currency_repo: CurrencyRepository, currency_code: str
) -> Currency:
...
def get_wallet_cur_balance(
    ...
    latest_snapshot: WalletSnapshot = snapshot_repo.get_latest_by_wallet_id(
        wallet_id=wallet_id
    )
    latest_balance: dict[Currency, Money] = latest_snapshot.balance

    missing_transactions: list[Transaction] = transaction_repo.get_all_after_snapshot(
        latest_snapshot
    )
